...
class LibraryController(QObject):
    def __init__(self,host):
        super().__init__(host); self.host=host; self.restoring=False; self.asset_id=None
    ...
    def activate(self,kind,ident):
        ...
        self.asset_id=ident;h.preview_revision+=1;h.editor.pause_preview();h.preview_timer.stop()
    ...
    def new_group(self,parent_id=None,name=None):
        ...
        if name is None:
            ...
            if not ok:return
        group=self.mutate(lambda lib:lib.add_group(name,parent_id),'New Group')
    ...
    def move(self,kind,ident,target,index=None):
        operation=(lambda lib:lib.move_group(ident,target,index)) if kind=='GROUP' else (lambda lib:lib.move_resource(ident,target,index))
